    ...
    <View className={`flex-1 ${backgroundTheme[theme]}`}>
      <View className="flex items-end justify-end">
        ...
        <View className="w-[200px] bg-white border rounded-xl mr-2 mt-2">
          <RNPickerSelect onValueChange={(value)=> {setselectedLanguage(value); i18n.changeLanguage(value);}}
            items={[
      ...
      <View className="flex-1 justify-center items-center gap-4 mb-20">
        <Text className={`${theme === 'dark' ? 'text-white' : 'text-black'} text-2xl font-semibold`}>Theme: {theme}</Text>
        <Text className={`${theme === 'dark' ? 'text-white' : 'text-black'} text-2xl font-semibold`}>Language: {selectedLanguage}</Text>
        <Text className={`${theme === 'dark' ? 'text-white' : 'text-black'} text-xl font-semibold`}>{t('greetings')}</Text>
      </View>
    </View>
  );
}
