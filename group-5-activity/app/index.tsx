...
const Card = ({
  ...
  return (
    <GestureDetector gesture={panGesture}>
      <Animated.View className='w-[220px] h-[320px] bg-white rounded-2xl justify-center items-center shadow-lg' style={animatedStyle}>
      <Text className={`text-4xl font-bold ${isRed ? 'text-red-600' : 'text-black'}`}>{rank}</Text>
        <Text className={`text-4xl mt-10 ${isRed ? 'text-red-600 ' : 'text-black'}`}>{suit}</Text>
      </Animated.View>
    </GestureDetector>
  );
};
      ...
      <View className="flex justify-center items-center mt-40">
        <GestureHandlerRootView>
          {card ? (
            ...
            <Text className="color-white text-[40px] italic mb-20">Deck finished</Text>
          )}
        </GestureHandlerRootView>
