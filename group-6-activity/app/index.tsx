import { Text, View, Pressable, ToastAndroid, TouchableOpacity } from "react-native";
...
export default function Index() {
  ...
  const handleTakePhoto = async () => {
    try {
      const [photo, currentLocation] = await Promise.all([
        cameraRef.current?.takePictureAsync({ quality: 0.5, base64: false, exif: false }),
  ...
  const handleRetakePhoto =() => {setPhoto(null), setLocation(null)};

  if (photo && location) return <PhotoPreviewSection photo={photo} location={location} handleRetakePhoto={handleRetakePhoto} />
  ...
  return (
    <View className="flex-1 justify-center bg-gray-200">
      ...
      <View className="flex flex-row justify-center items-center mt-10 gap-10">
        <TouchableOpacity className="bg-white rounded-full border w-[60px] h-[60px] items-center justify-center" onPress={toggleCamera}>
          <Icon name='retweet' size={24} color='black'/>
        </TouchableOpacity>
        <TouchableOpacity className="bg-white rounded-full border w-[60px] h-[60px] items-center justify-center" onPress={handleTakePhoto}>
